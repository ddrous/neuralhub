...
plt.ylim(-2, 2)
plt.draw();
plt.savefig(f"data/duffing_phase_space.png", dpi=300, bbox_inches='tight')
...
class Processor(eqx.Module):
    ...
    def __init__(self, data_size, int_size, context_size, key=None):
        keys = generate_new_keys(key, num=5)
        self.neuralnet = eqx.nn.MLP(in_size=data_size+1, 
                                    out_size=data_size,
                                    width_size=16,
                                    depth=3, 
                                    key=keys[0])
    ...
    def __call__(self, x, t):
        xt = jnp.concatenate([x, jnp.array([t])], axis=0)
        return self.neuralnet(xt)
...
# model = eqx.tree_deserialise_leaves("data/model_03.eqx", model)
# params, static = eqx.partition(model, eqx.is_array)
...
ax.set_xlabel(f'Displacement ($x$)')
ax.set_ylabel(f'Velocity ($y$)')
ax.set_title('Phase Space')
...
eqx.tree_serialise_leaves("data/model_ml4de.eqx", model)
...
plt.xlabel(r'Displacement ($x$)')
plt.ylabel(r'Velocity ($y$)')
...
for i in range(X.shape[0]):
    if i==0:
        sbplot(X_hat[i, :,0], X_hat[i, :,1], "+-", x_label='x', y_label='y', label=f'Pred', title=f'Phase space Prediction', ax=ax, alpha=0.5, color=colors[i])
        # sbplot(X[i, :,0], X[i, :,1], "o", lw=1, label=f'True', ax=ax, color=colors[i])
    else:
        sbplot(X_hat[i, :,0], X_hat[i, :,1], "+-", x_label='x', y_label='y', ax=ax, alpha=0.5, color=colors[i])
        # sbplot(X[i, :,0], X[i, :,1], "o", lw=1, ax=ax, color=colors[i])
...
# plt.savefig(f"data/test_simple.png", dpi=300, bbox_inches='tight')
    ...
    plt.ylim(-2, 2)
    plt.draw();
    plt.savefig(f"data/test_model.png", dpi=300, bbox_inches='tight')
