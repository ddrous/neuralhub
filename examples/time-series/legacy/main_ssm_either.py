...
nb_epochs = 25000
batch_size = 256
grounding_length = 50      ## The length of the grounding pixel for the autoregressive digit generation
    ...
    def __call__(self, xs):
        ...
        def forward(xs_):
            ...
            if self.inference_mode:
                ts_ = ts_full[:grounding_length]
                xs_ = xs_[:grounding_length]
            else:
                ts_ = ts_full
                xs_ = xs_
            (thet_final, _), (xs_true, ) = jax.lax.scan(f, (self.theta, ts_[0]), (xs_, ts_))
            ...
            return eqx.filter_vmap(root_fun)(ts_full)
