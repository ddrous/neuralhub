...
class Ses2Seq(eqx.Module):
    ...
    def __call__(self, xs):
        ...
        def forward(xs_):
            ## Fine-tune the latents weights wihile decoding them
            def f(carry, input_signal):
                thet, x_hat = carry
                x_true, t_true = input_signal

                if self.inference_mode:
                    x_t = jnp.where(t_true > grounding_length/seq_length, x_hat, x_true)
                else:
                    x_t = x_true
                ...
                model_out = root_fun(t_true)
                x_hat_next = model_out[:self.data_size]
                x_class_next = model_out[self.data_size:]

                return (thet_next, x_hat_next), (x_hat_next, x_class_next)

            ## Call the JAX scan
            ts_ = jnp.linspace(0, 1, xs_.shape[0])[:, None]
            (_, _), (x_recons, x_classes) = jax.lax.scan(f, (self.theta, xs_[0]), (xs_, ts_))
