...
class Ses2Seq(eqx.Module):
    ...
    def __init__(self, 
        ...
        self.decoder = eqx.nn.Linear(latent_size, out_size, use_bias=False, key=keys[1])
    ...
    def __call__(self, xs, ts, aux):
        ...
        def forward(xs_, ts_, k_):
            ...
            def f(carry, input_signal):
                ...
                if full_matrix_A:
                    if use_theta_prev:
                        ...
                        thet_next = self.cell(x_t, thet)        ## LSTM, use xprev ?
                else:
                    pass

                ## 2. Decode the latent space
                y_next = final_activation(self.decoder(thet_next[1]))
            ...
            (thet_final, _, _, _, _), (xs_, ) = jax.lax.scan(f, 
                                                             ((init_hidden, init_hidden), xs_[0], -ts_[1:2], xs_[0], 0), 
                                                             (xs_, ts_[:, None], keys))
