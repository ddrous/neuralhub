...
context_pool_size = 128*4  ## NUmber of (t, x) pairs to compute the representation
...
class Ses2Seq(eqx.Module):
    ...
    def __call__(self, xs, ts, aux):
        ...
        def forward(xs_, ts_, k_):
            ...
            return xs_, latent_representation
...
def loss_fn(model, batch, utils):
    ...
    if supervision_task=="classification":
        ...
        X_classes, _ = model(X_true, times, utils)     ## Y_hat: (batch, time, num_classes) 
        ...
        X_recons, _ = model(X_true, times, utils)     ## Y_hat: (batch, time, data_size) 
        alpha, key = utils
        ...
        X_full, _ = model(X_true, times, utils)     ## Y_hat: (batch, time, data_size+nb_classes)
...
for i, batch in enumerate(testloader):
    ...
    if supervision_task == "classification":
        X_classes, _ = model(X_true, times, (alpha, test_key))
        pred_logits = X_classes[:, -1, :]   ## We only care about the last prediction: (batch, num_classes)
        acc = jnp.mean(jnp.argmax(pred_logits, axis=-1) == X_labels)
        accs.append(acc)

    elif supervision_task == "reconstruction":
        X_recons, _ = model(X_true, times, (alpha, test_key))
        if not use_mse_loss:
        ...
        X_full, _ = model(X_true, times, (alpha, test_key))
        recons_size = data_size if use_mse_loss else 2*data_size
        X_recons, X_classes = X_full[:, :, :recons_size], X_full[:, :, recons_size:]
...
if not supervision_task=="classification":
    ...
    xs_recons, latents = model(xs_true, times, (alpha, test_key))
...
## Let's do a PCA on the latents and plot the first 2 components

visloader = NumpyLoader(testloader.dataset, batch_size=2**10, shuffle=True)
batch = next(iter(visloader))
(xs_true, times), labels = batch
xs_recons, latents = eqx.filter_jit(model)(xs_true, times, (alpha, test_key))
...
print("Latents shape after reduction:", latents_reduced.shape)

fig, ax = plt.subplots(1, 1, figsize=(10, 10))
ax.scatter(latents_reduced[:, 0], latents_reduced[:, 1], c=labels, cmap='tab10')
plt.draw();
plt.savefig(run_folder+"pca_latents.png", dpi=100, bbox_inches='tight')
