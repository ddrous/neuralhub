...
grounding_length = 1024       ## The length of the grounding pixel for the autoregressive digit generation
        ...
        def forward(xs_, ts_, k_):
            ...
            for i in range(nb_rnn_layers):
                ...
                def f(carry, input_signal):
                    ...
                    return (thet_next, x_next_mean, t_curr, x_prev, step+1), (y_next, thet)

                sup_signal = xs_ if not final_layer else xs_orig        ## Supervisory signal
                (thet_final, _, _, _, _), (xs_, thetas) = jax.lax.scan(f, (self.thetas[i], sup_signal[0], -ts_[1:2], sup_signal[0], 0), (sup_signal, ts_[:, None], keys))
...
if not supervision_task=="classification":
    ...
    # fig, axs = plt.subplots(1, 1*nb_cols, figsize=(4*3, 4), sharex=True, constrained_layout=True)
    # axs = axs.reshape(1, 1*nb_cols)

    batch = next(iter(visloader))
    (xs_true, times), labels = batch
    xs_recons_all = model(xs_true, times, (alpha, test_key))

    ## A plotting function for time step t
    def plot_recons(t, axs):
        xs_recons = xs_recons_all[t]
        axs = axs.reshape(1, 1*nb_cols)
        ...
        for i in range(1):
            for j in range(1):
                ...
                if i==0:
                    ...
                    axs[i, nb_cols*j+1].set_title(f"$t={t}$", fontsize=40)
        ...
        # plt.draw();
        # plt.savefig(run_folder+"recons_through_time.png", dpi=100, bbox_inches='tight')



    nb_plot_steps = 10
    fig, axs = plt.subplots(nb_plot_steps, nb_cols, figsize=(4*3, nb_plot_steps*4), sharex=True, constrained_layout=True)

    for t_, t in enumerate(range(1, 32*32, 32*32//nb_plot_steps)):
        print("t value:", t)
        # plot_recons(t, axs[:, t*nb_cols:(t+1)*nb_cols])
        plot_recons(t-1, axs[t_-1, :])
...
if not supervision_task=="classification":
    ...
    ## Take the diagonal along the last two axes
    xs_recons = jnp.diagonal(xs_recons, axis1=-2, axis2=-1)
    ...
    plt.suptitle(f"Reconstruction using {grounding_length} initial pixels", fontsize=65)
    plt.draw();
    plt.savefig(run_folder+"reconstruction_new.png", dpi=100, bbox_inches='tight')



#%%

## Plot the first datapoint in the recons along the time axis
fig, ax = plt.subplots(1, 1, figsize=(20, 10))
if not use_mse_loss:
    print("Print all shapes:", xs_recons_all.shape)
    plot_t = np.random.randint(0, xs_recons_all.shape[1])
    xs_uncert = xs_recons_all[0, plot_t, :, data_size:].squeeze().T
    xs_recons = xs_recons_all[0, plot_t, :, :data_size].squeeze().T

    print("Shapes of the recons and uncertainty:", xs_recons.shape, xs_uncert.shape)

    ax.plot(xs_recons, label="Reconstruction")
    # ax.plot(xs_uncert, label="Uncertainty")
    ax.set_title("Reconstruction and Uncertainty")
    plt.legend()
    plt.draw();
    plt.savefig(run_folder+"time_dim.png", dpi=100, bbox_inches='tight')
