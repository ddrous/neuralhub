...
results = jsp.optimize.minimize(loss_fn_, params_flat, args=(static, (data[0,:,...], t_eval)), tol=1e-10, method='BFGS', options={'maxiter': 1000})

wall_time = time.time() - start_time
time_in_hmsecs = seconds_to_hours(wall_time)
print("\nTotal BFGS training time: %d hours %d mins %d secs" %time_in_hmsecs)


print("\nOptimisation result: success, nit, loss_val: ", results.success, results.nit, results.fun)
...
# params_flat, _, errors, nit = newton(params_flat, loss_fn_, gf, hf, tol=1e-10, maxiter=200, lr=1e-1, lr_decr=0.999)
...
# # Preliminary results
# - GD is slow and poor
# - BFGS is fast, but seems to stop too early saying Failure, while it looks like success. I believe if do BFGS in a loop, we might get absolutely great results
# - Newton is weird, for now !!
