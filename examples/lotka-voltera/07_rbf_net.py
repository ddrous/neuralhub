...
class Processor(eqx.Module):
    ...
    def __init__(self, in_size=2, nb_centers=10, out_size=2, key=None):
        ...
        self.augmentation = Augmentation(in_size, nb_centers, out_size, key=keys[1])

    def __call__(self, x, t):
        # return self.physics(x, t) + self.augmentation(x, t)
        return self.augmentation(x, t)

# %%

model_keys = get_new_key(SEED, num=2)

model = Processor(in_size=2, nb_centers=500, out_size=2, key=model_keys[0])
...
# # Preliminary results
# - RBF is NOT better than MLP. At least one order of mangitude in the loss.
