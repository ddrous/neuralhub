...
data = train_data[None, :, ::10, :]
T_horizon = 1.
## Create 
t_eval = np.linspace(0, T_horizon, data.shape[2])
print("Data shape:", data.shape)

test_data = test_data[None, :, ::10, :]
...
class NeuralODE(eqx.Module):
    ...
    def __call__(self, x0s, t_eval, key):
        ...
        return x_recons, x_factors, (z0s_mu, z0s_logvar)
...
def loss_fn(model, batch, key):
    ...
    return jnp.mean(rec_loss + KL_loss)
...
for epoch in range(nb_epochs):
    ...
    for i in range(0, nb_data_points, batch_size):
        ...
        model, opt_state_node, loss = train_step(model, batch, opt_state_node, train_key)
...
for i in range(10):     ## Plot 10 trajectories
    if i==0:
