...
print_every = 10
nb_epochs = 300
batch_size = 2*128              ## Two trajectories per environment are picked for each train_step; this results in 45 contrastive pairs (9 pos, 36 negs). This process is repeated once. 
...
if train == True:
    ...
    ## Save the results
    np.save("data/losses_07.npy", losses)
    np.save("data/nb_steps_07.npy", nb_steps)
    np.save("data/xis_07.npy", xis)
    np.save("data/init_xis_07.npy", init_xis)

    model = eqx.combine(params, static)
    eqx.tree_serialise_leaves("data/model_07.eqx", model)

else:
    losses = np.load("data/losses_07.npy")
    nb_steps = np.load("data/nb_steps_07.npy")
    xis = np.load("data/xis_07.npy")
    init_xis = np.load("data/init_xis_07.npy")

    model = eqx.combine(params, static)
    model = eqx.tree_deserialise_leaves("data/model_07.eqx", model)
